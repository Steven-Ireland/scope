      ...
      <header className="border-b p-4 flex items-center justify-between bg-card shrink-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold">
            {isNew ? 'Add New Server' : `Server Settings: ${form.name}`}
          </h1>
        </div>
        {!isNew && (
      ...
      <main className="flex-1 p-8 max-w-2xl mx-auto w-full space-y-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {isNew
                ? 'Enter the details for your new Elasticsearch cluster.'
          ...
          <CardContent className="space-y-4">
              ...
              {(['certPath', 'keyPath'] as const).map((f) => (
                <div key={f} className="flex gap-2 items-end">
                  <div className="grid gap-2 flex-1">
                    <Label>{f === 'certPath' ? 'Client Certificate (CRT)' : 'Client Key'}</Label>
                    <Input
                      value={form[f] || ''}
            ...
            <div className="border-t pt-4 space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                Define patterns (e.g., <code>logs-*</code>) to group date-based indices.
              </p>
          ...
          <CardFooter className="border-t p-6 mt-4 flex justify-between">
            {isNew ? (
              <Button onClick={handleCreate} disabled={!form.name || !form.url} className="ml-auto">
                Create Server
              </Button>
            ) : (
              <Button
